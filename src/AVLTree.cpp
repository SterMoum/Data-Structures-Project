...
void AVLTree::addLeaf(std::string key) //Call the private function.
{
    root = addLeafPrivAVL(key,root);
}

node* AVLTree::addLeafPrivAVL(std::string key, node* pointer) //Add a leaf to the AVL tree.
{
    if (pointer == NULL) //If the node is NULL.
    {
    ...
    pointer->height = max(Height(pointer->left), Height(pointer->right))+1; //Sett to the height, the max height of left or right child and add 1.
...
node* AVLTree::RightRotation(node* &pointer) //Right rotation
{
    if (pointer->left != NULL) //If the left child isn't NULL.
    {
        ...
        pointer->left = x->right; //Sett the left child of the current node to right child of  this node.
        x->right = pointer; //Sett the right child of the left child to the current node.
        pointer->height = max(Height(pointer->left), Height(pointer->right)) + 1; //Sett to the pointer height, the max height of left or right child and add 1.
        x->height = max(Height(x->left), pointer->height) + 1; //Sett to the right child(x) of the left child(pointer->left) of the current node(pointer) height,
        return x;  //return x node.                            // the max height of left child of x node and pointer height and add 1.
    }
    return pointer;
...
node* AVLTree::LeftRotation(node* &pointer) //Left rotation.
{
    if (pointer->right != NULL)
    {
        ...
        pointer->right = x->left; //Sett the right child of the current node to left child of this node.
        x->left = pointer; //Sett the left child of the left child to the current node.
        pointer->height = max(Height(pointer->left), Height(pointer->right)) + 1; //Sett to the pointer height, the max height of left or right child and add 1.
        x->height = max(Height(pointer->right), pointer->height) + 1; //Sett to the left child(x) of the right child(pointer->right) of the current node(pointer) height,
        return x;
    }
    return pointer;
}
...
{
    pointer->left = LeftRotation(pointer->left); //Sett the left child to left rotation.
    return RightRotation(pointer); //right rotation.
}

node* AVLTree::doubleLeftRotation(node* &pointer) //Double left rotation.
{
    pointer->right = RightRotation(pointer->right); //Sett the right child to right rotation.
...
void AVLTree::FindNode(std::string key) //Call the private function.
{
    //node* temp;
    FindNodePriv(key,root);
    //if (temp!=NULL)
        //cout<<temp->key<<"["<<temp->count<<"]"<<endl;                             // if you want to show the results
}

void AVLTree::DeleteNode(std::string key) //Call the private function.
{
    root=DeleteNodePrivAVL(key,root);
}

node* AVLTree::DeleteNodePrivAVL(std::string key, node* pointer) //Delete function.
{
    ...
    else if (pointer->left!=nullptr && pointer->right!=nullptr)
    {
        ...
        pointer->key = temp->key; //sett to pointer key the temp key.
        pointer->count=temp->count; //sett to pointer counter the temp counter.
...
void AVLTree::WriteToFileAVL(std::string* Q,int Q_counter)
{
    node* pointer;

    std::ofstream f_out;

    f_out.open("AVLTreeResults.txt");

    if(f_out.is_open())
    {
        for(int i = 0; i < Q_counter; i++)
        {
            pointer = FindNodePriv(Q[i],root);

            if(pointer != nullptr)
                f_out << pointer << std::endl;
