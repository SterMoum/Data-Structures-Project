...
Tree::Tree() // Empty Constructor.
{
    root=NULL; //Set NULL to root.
}

node* Tree::CreateLeaf(std::string key) //Create a new Node.
{
...
void Tree::addLeaf(std::string key) //Call the private function.
{
    addLeafPriv(key, root);
}

void Tree::addLeafPriv(std::string key, node* pointer) //Add a leaf to the tree.
{
    if (pointer==NULL)
    {
...
void Tree::FindNode(std::string key) //Call the private function.
{
    //node* temp;
    //temp=FindNodePriv(key,root);
    FindNodePriv(key,root);
    /*
    if (temp!=NULL)                                                   // if you want to show the results in cmd
...
node* Tree::FindNodePriv(std::string key, node* pointer)
{
    if (pointer!=NULL) //If the current node exists
    {
        ...
        else //Otherwise.
        {
            ...
            {
                return FindNodePriv(key,pointer->left); //Go left
            }
            else //Otherwise.
            {
                return FindNodePriv(key,pointer->right); //Go right.
            }
        }
...
void Tree::DeleteNode(std::string key) //Call the private function.
{
    DeleteNodePriv(key, root);
}

void Tree::DeleteNodePriv(std::string key, node* pointer)
{
    if (root!=NULL) //If the tree is not empty;
    {
        ...
        {
            std::cout<<"The Word "<< key <<" didn't found in the tree!"<< std::endl;
        }
    }
...
void Tree::DeleteRoot() // Delete the root.
{
    ...
    std::string TempMinKey; //A temporarily variable to save the min key.
...
void Tree::InOrderPriv(node* pointer) //InOrder function.
{
    if (pointer!=NULL) //If the current node isn't NULL.
    {
        ...
        std::cout<<pointer->key<<"["<<pointer->count<<"]"<< std::endl; //print the node's key, counter.
...
void Tree::WriteToFileTree(std::string* Q,int Q_counter)
{
    node* pointer;

    std::ofstream Fout;

    Fout.open("BinTreeResults.txt");
...
std::ostream &operator << (std::ostream& out,node* pointer)
{
    out << pointer->key << "[" << pointer->count << "]";
